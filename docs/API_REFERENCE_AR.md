...
- اضبط المتغير البيئي `API_KEYS` بقيم مفصولة بفواصل تربط كل مفتاح بالاسم والبريد، مثال: `API_KEYS='key1:Ali <ali@example.com>, key2:Sara <sara@example.com>'`.
- يتم إنشاء سجل `User` تلقائيًا لكل مفتاح (إن لم يكن موجودًا) ويتم ربط كل مهمة ضغط بالمستخدم الموافق.
- يجب تضمين الترويسة `X-API-Key` في كل طلب عند تفعيل المفاتيح، ويبقى الوصول مفتوحًا عند ترك المتغير فارغًا.
...
| `API_KEYS` | صيغة `key:Name <email>` لتحديد المفاتيح والتحكم في هوية المستخدم المرتبط بكل مهمة. |
