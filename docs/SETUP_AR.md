...
يفترض أن يظهر رقم الإصدار (مثلاً `10.02.1`). على Windows قد يكون اسم التنفيذ `gswin64c` أو `gswin32c`. التطبيق يحاول اكتشاف الأسماء الثلاثة تلقائيًا (`gs`, `gswin64c`, `gswin32c`) بالإضافة إلى البحث في مسارات التثبيت الشائعة داخل `Program Files` إذا لم يكن التنفيذ متوفرًا ضمن `PATH`.
